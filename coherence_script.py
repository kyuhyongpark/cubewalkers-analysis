...
# %%
def simulate_model_coherence(models, sync=True, W=2500):
    coherences = {}
...
# %%
sync_coherences=simulate_model_coherence(sync_models,sync=True)

# %%
async_coherences=simulate_model_coherence(async_models,sync=False)

# %%
with open('./data/cohrence_values.csv','w') as f:
    ...
    for model_name, smodel, amodel in [(k,sync_models[k],async_models[k]) for k in sorted(async_models)]:
        f.write(f'{model_name},{cp.round(sync_coherences[model_name],3)},{cp.round(async_coherences[model_name],3)}\n')
